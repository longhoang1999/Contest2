...
class ExportKhuonvien implements FromCollection, WithHeadings, WithEvents
// class AdmissionsExport implements FromView
{
    ...
    public function collection()
    {
        ...
        foreach($this->id as $key => $value){
            $chiso = DB::table('khuonvien_chiso')->where("id", $value)->first();
            $row = [
                $key + 1 ,
                $chiso->code,
                $chiso->khuonvien,
    ...
    public function registerEvents(): array
    {
        return [
            AfterSheet::class    => function(AfterSheet $event) {
                //Sheet name
                $event->sheet->getDelegate()->setTitle("Khuôn viên TSC và PH " . $this->nam);
                // All headers
