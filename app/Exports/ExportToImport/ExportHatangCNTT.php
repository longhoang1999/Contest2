...
class ExportHatangCNTT implements FromCollection, WithHeadings, WithEvents
// class AdmissionsExport implements FromView
{
    ...
    public function collection()
    {
        ...
        foreach($this->id as $key => $value){
            $chiso = DB::table('hatangcntt_chiso')->where("id", $value)->first();
            $row = [
                $key + 1 ,
                $chiso->code,
                $chiso->chisotk,
    ...
    public function registerEvents(): array
    {
        return [
            AfterSheet::class    => function(AfterSheet $event) {
                //Sheet name
                $event->sheet->getDelegate()->setTitle("Hạ tầng CNTT " . $this->nam);
                // All headers
