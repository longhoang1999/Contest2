...
class ExportCongtrinh implements FromCollection, WithHeadings, WithEvents
// class AdmissionsExport implements FromView
{
    ...
    public function collection()
    {
        ...
        foreach($this->id as $key => $value){
            $chiso = DB::table('congtrinhdt_chiso')->where("id", $value)->first();
            $row = [
                $key + 1 ,
                $chiso->code,
                $chiso->congtrinh,
    ...
    public function headings() :array {
        return [
            ...
            "Diện tích làm việc cho giảng viên (m2)",
            "Địa chỉ"
        ];
    }
    ...
    public function registerEvents(): array
    {
        return [
            AfterSheet::class    => function(AfterSheet $event) {
                ...
                $event->sheet->getDelegate()->getStyle("H")->getActiveSheet()->getColumnDimension('I')->setWidth(30);
