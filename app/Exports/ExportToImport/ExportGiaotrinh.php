...
class ExportGiaotrinh implements FromCollection, WithHeadings, WithEvents
// class AdmissionsExport implements FromView
{
    ...
    public function collection()
    {
        ...
        foreach($this->id as $key => $value){
            $chiso = DB::table('giaotrinhck_chiso')->where("id", $value)->first();
            $row = [
                $key + 1 ,
                $chiso->code,
                $chiso->chisotk,
    ...
    public function registerEvents(): array
    {
        return [
            AfterSheet::class    => function(AfterSheet $event) {
                //Sheet name
                $event->sheet->getDelegate()->setTitle("Giáo trình sách CK " . $this->nam);
                // All headers
